...
if (!is.null(params$gwas_id) & !is.null(params$output_dir)) {
  # api_data <- params$gwas_id %>% process_api_info()
  api_data <- path(params$output_dir, glue("{params$gwas_id}.json")) %>% jsonlite::read_json() %>% tibble::enframe()
} else {
  api_data <- NULL
}
trait_name <- get_trait_name(api_data)
metadata <- jsonlite::read_json(params$metadata_file)
qc_metrics <- jsonlite::read_json(params$qc_metrics_file)
